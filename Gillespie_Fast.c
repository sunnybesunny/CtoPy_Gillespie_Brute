...
void Gillespie_Fast(double t_range[], double V_curr[][Type_spe], double C_curr[], double H_curr[],double S_curr[][Type_dose], double P_curr[], FILE * f, int num_RXN_curr,int scheme) {
		...
		double tau; /* next rxn time*/
		double r; /*random # for reaction*/
		double sum_prop= 0.0;	/*sum of propencities*/
//		unsigned int SEED;
		int reaction;
		double random;/*random # draw from uniform distribution for next rxn time*/
		int count=0;
		unsigned long long init[4]={0x12345ULL, 0x23456ULL, 0x34567ULL, 0x45678ULL}, length=4;
		init_by_array64(init, length);
		int i;
